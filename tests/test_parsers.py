...
def test_bean_parser(water_mol):
    assert parse_bean(prefix + "assets/water.bean") == water_mol
...
def test_cif_parser(water_mol):
    assert parse_cif(prefix + "assets/water.cif") == water_mol


def test_cif_extra_info(water_mol):
    assert parse_cif(prefix + "assets/water_extra_info.cif") == water_mol


def test_pdb_parser(water_mol):
    assert parse_pdb(prefix + "assets/water.pdb") == water_mol


def test_pdb_extra_info(water_mol):
    assert parse_pdb(prefix + "assets/water_extra_info.pdb") == water_mol
